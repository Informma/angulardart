...
/// `comp.a = newA` (and call `AfterChanges.ngAfterChanges`, if implemented).
///
/// When [DoCheck] is implemented, AngularDart will invoke `ngDoCheck()` every
/// time the component was checked for changes, regardless of whether the
/// identity of an `@Input()` has changed, giving the user the opportunity to
/// perform manual checks for mutations.
///
/// The `ngDoCheck` lifecylce is used by `NgFor`, `NgStyle` and `NgClass`, for
