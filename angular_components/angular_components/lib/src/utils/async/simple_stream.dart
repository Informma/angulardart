...
class SimpleStream<T> extends Stream<T> implements EventSink<T> {
  ...
  StreamSubscription<T> listen(void Function(T)? onData,
    ...
    var sub = SimpleStreamSubscription<T>(
        this, onData, onDone, onError, cancelOnError ?? false, contextZone);
...
class EmptySimpleStream<T> extends SimpleStream<T> {
  ...
  StreamSubscription<T> listen(void Function(T)? onData,
      {Function? onError, void Function()? onDone, bool? cancelOnError}) {
    return SimpleStreamSubscription._empty() as StreamSubscription<T>;
  }
}
  ...
  void _add(T data) {
    if (_callback != null) {
      if (_contextZone != null) {
        _contextZone!.runUnary(_callback as dynamic Function(dynamic), data);
      } else {
        _callback!(data);
      }
    }
