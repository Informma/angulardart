...
class MaterialPopupComponent extends Object
  ...
  void _stopRepositionLoop() {
    ...
    if (_repositionOffsetX != 0 || _repositionOffsetY != 0) {
      _overlayRef!.state.left = (_overlayRef!.state.left ?? 0) + _repositionOffsetX;
      _overlayRef!.state.top = (_overlayRef!.state.top ?? 0) + _repositionOffsetY;
  ...
  void _reposition(double _) {
    ...
    if (state.constrainToViewport) {
      ...
      _repositionOffsetX += viewportShift.left.round();
      _repositionOffsetY += viewportShift.top.round();
    }
...
Stream<List<T?>> _mergeStreams<T>(List<Stream<T>> streams) {
  var streamSubscriptions = List<StreamSubscription<T>?>.filled(streams.length, null);
