...
class NgScanner {
  ...
  NgToken? scanAfterComment() {
    ...
    return handleError(
      ParserErrorCode.unterminatedComment,
  ...
  NgToken? scanAfterElementDecorator() {
    ...
    if (type == NgSimpleTokenType.eof ||
      ...
      return handleError(
        ParserErrorCode.expectedTagClose,
        lastToken.offset,
        lastToken.length,
      );
    }
    ...
    if (type == NgSimpleTokenType.doubleQuote ||
      ...
      var lastToken = _lastToken!;
      return handleError(
        ParserErrorCode.expectedEqualSign,
        lastToken.offset,
  ...
  NgToken? scanAfterElementIdentifierClose() {
    ...
    if (type == NgSimpleTokenType.commentBegin ||
      ...
      var lastToken = _lastToken!;
      return handleError(
        ParserErrorCode.expectedAfterElementIdentifier,
        lastToken.offset,
        lastToken.length,
      );
    }
    ...
    if (type == NgSimpleTokenType.voidCloseTag) {
      return handleError(
        ParserErrorCode.voidCloseInCloseTag,
        current.offset,
        current.length,
      );
    }
  ...
  NgToken? scanElementDecorator() {
    ...
    if (type == NgSimpleTokenType.equalSign ||
      ...
      var lastToken = _lastToken!;
      return handleError(
        ParserErrorCode.elementDecorator,
        lastToken.offset,
        lastToken.length,
      );
    }
    ...
    if (type == NgSimpleTokenType.closeBracket ||
        ...
        type == NgSimpleTokenType.closeBanana) {
      return handleError(
        ParserErrorCode.elementDecoratorSuffixBeforePrefix,
        current.offset,
        current.length,
      );
    }
  ...
  NgToken? scanElementDecoratorValue() {
    ...
    if (type == NgSimpleTokenType.identifier) {
      return handleError(
        ParserErrorCode.elementDecoratorValueMissingQuotes,
        current.offset,
        current.length,
      );
    }
    ...
    if (type == NgSimpleTokenType.openBracket ||
        ...
        type == NgSimpleTokenType.atSign) {
      return handleError(
        ParserErrorCode.elementDecoratorValue,
  ...
  NgToken? scanElementIdentifier({bool wasOpenTag = false}) {
    ...
    if (wasOpenTag) {
      if (type == NgSimpleTokenType.openBracket ||
          ...
          type == NgSimpleTokenType.singleQuote) {
        return handleError(
          ParserErrorCode.elementIdentifier,
  ...
  NgToken? scanInterpolation() {
    ...
    if (type == NgSimpleTokenType.mustacheEnd) {
      return handleError(
        ParserErrorCode.emptyInterpolation,
        _lastOpenMustacheOffset,
...
///   Element `Open` indicates a tag opening (`<blah>`).
///   Element `Close` indicates a tag closing (`</blah>`).
///
/// Start indicates the left bracket (opening angle bracket or `< /`).
/// End indicates the right bracket (closing angle bracket `>` or `/ >`).
enum NgScannerState {
  hasError,
  isEndOfFile,
  scanAfterComment,
