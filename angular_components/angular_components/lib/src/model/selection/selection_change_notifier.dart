...
class _SelectionChangeRecordImpl<T> extends ChangeRecord
  ...
  factory _SelectionChangeRecordImpl({Iterable<T>? added, Iterable<T>? removed}) {
    final Iterable<T> addedList = added != null ? UnmodifiableListView(added) : <T>[];
    final Iterable<T> removedList = removed != null ? UnmodifiableListView(removed) : <T>[];
    return _SelectionChangeRecordImpl._(addedList, removedList);
  }
