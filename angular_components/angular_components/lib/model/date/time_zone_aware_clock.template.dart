// **************************************************************************
// Generator: AngularDart Compiler
// **************************************************************************

import 'time_zone_aware_clock.dart';

import 'package:angulardart_components/model/date/time_zone_aware_clock.dart' as _i1;
import 'package:angulardart/src/reflector.dart' as _ngRef;
import 'package:angulardart/di.template.dart' as _ref0;

// ignore_for_file: no_leading_underscores_for_library_prefixes
var _visited = false;
void initReflector() {
  if (_visited) {
    return;
  }
  _visited = true;

  _ngRef.registerFactory(TimeZoneAwareClock, (_i1.SettableTimeZone p0) => TimeZoneAwareClock.new(p0));
  _ngRef.registerDependencies(TimeZoneAwareClock, const [
    [_i1.SettableTimeZone],
  ]);
  _ngRef.registerDependencies(clockFactory, const [
    [_i1.SettableTimeZone],
  ]);
  _ref0.initReflector();
}
