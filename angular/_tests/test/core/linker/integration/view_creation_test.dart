...
void main() {
  ...
  test('should support moving embedded views', () async {
    final template = (web.document.createElement('template') as web.HTMLTemplateElement)..append(web.document.createElement('div'));
    final testBed = NgTestBed(
  ...
  group('property bindings', () {
    ...
    test("shouldn't be created when a directive property has the same name",
      ...
      expect((span as web.HTMLElement).title, isEmpty);
    });
    ...
    test('should allow directive host property to update DOM', () async {
      ...
      final testFixture = await testBed.create();
      final span = testFixture.rootElement.querySelector('span');
      expect((span as web.HTMLElement).title, 'TITLE');
    });
  });
  ...
  group('property decorators', () {
    ...
    test('should support @HostBinding', () async {
      ...
      expect(directiveElement.getAttribute('my-attr'), 'bar');
    });
    ...
    test('should support @HostListener', () async {
      ...
      final directiveElement = testFixture.rootElement.children.item(0)!;
      directiveElement.dispatchEvent(web.MouseEvent('click'));
...
class SimpleImperativeViewComponent {
  SimpleImperativeViewComponent(web.Element hostElement) {
    hostElement.append(web.Text('hello imp view'));
  }
}
...
class DirectiveWithPropDecorators {
  ...
  void onClick(web.Element target) {
    this.target = target;
  }
