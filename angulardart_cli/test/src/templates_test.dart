...
void main() {
  ...
  group('Templates consistency', () {
    ...
    test('all pubspec templates omit the web dependency (dart:html base)', () {
      final pubspecs = [
        Templates.projectPubspec,
        Templates.projectPubspecSeo,
        Templates.projectPubspecSsr,
        Templates.projectPubspecHybrid,
        Templates.projectPubspecSsrSeo,
      ];
      for (final content in pubspecs) {
        expect(
          content,
          isNot(contains('web:')),
          reason: 'Pubspec template should not depend on package:web: $content',
        );
      }
    });

    test('projectPlatformDomBrowser re-exports dart:html (not package:web)', () {
      expect(Templates.projectPlatformDomBrowser, contains("export 'dart:html';"));
      expect(Templates.projectPlatformDomBrowser, isNot(contains('package:web')));
    });
