...
abstract mixin class MenuItemMixin implements _MenuItemBase {
  bool get hasIcon => icon != null;
...
class ActiveMenuItemModel<T> extends ActiveItemModel<T> {
  ...
  static CombinedList<T> _createEnabledItemGroupList<T>(
      List<MenuItemGroup<T>>? menuGroups, bool filterOutUnselectableItems) {
    if (menuGroups == null) return CombinedList<T>([]);

    if (!filterOutUnselectableItems) return CombinedList<T>(menuGroups as List<List<T>>);
